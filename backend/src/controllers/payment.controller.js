...
// Removed the import for Razorpay instance since it's no longer needed
...
const coursePayment = asyncHandler(async (req, res) => {
  ...
  // Assuming you have other logic to handle payments without Razorpay
  // const options = {
  ...
  return res.status(200).json(new ApiResponse(200, {}, "order fetched"));
});

const getkey = asyncHandler(async (req, res) => {
  return res.status(200).json(new ApiResponse(200, { key: process.env.KEY_ID }, "razor key fetched"));
});
...
const coursePaymentConfirmation = asyncHandler(async (req, res) => {
  ...
  if (isAuthentic) {
    ...
    return res.status(200).json(new ApiResponse(200, { orderDetails }, "payment confirmed"));
  } else {
